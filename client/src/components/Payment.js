...
const PaymentForm = () => {
  useEffect(() => {
    toast.info(
      "This is a demo payment, so don't share original bank details!",
      {
        autoClose: false,
      }
    );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        await axios
          .post(`${process.env.REACT_APP_SERVER_URL}/addToOrders`, {
            cookieVal,
            arr,
            isProductFromCart,
          })
  ...
  return (
    <div className="grid place-items-center">
      ...
      <form
        ...
        className="mb-10 w-[98vw] lg:w-[60vw] grid place-items-center"
      >
        <img
          className="w-4/12 lg:w-3/12 my-3  mr-auto "
          src={require("./Images/card_accept.jpg")}
          alt=""
        />
        ...
        <input
          type="submit"
          className="my-3 cursor-pointer text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg"
          value={`Pay ₹ ${counter.SmallCartPreviewTotal}.00`}
        />
      </form>
